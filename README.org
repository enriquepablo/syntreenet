...
With duoligno, the grammar for the facts (and thus for the conditions and
...
grammar can be plugged in to duoligno. This allows us to abstract away the
...
I want to stress this: duoligno knows nothing about syntax. Facts are a black box
...
So, this is how we'd do it with duoligno, using the grammar file provided in
babel.ont (and shortening "belongs to" to "isa", and "subset of" to "is"):

    from duoligno.ruleset import Rule, KnowledgeBase
    from duoligno.babel.ont import Word, F, isa, is_
...
scripts submodule, which test both CLIPS and duoligno with the animal ontology
 ...
 * We are also hitting here a sweet spot for CLIPS, with just 2 rules and just
   2 conditions in each. Due to the different architecture duoligno does not
...
new fact for CLIPS, and a leveling out of the cost for duoligno.
...
part of the structure of the facts that can leak to duolingo, since it is
